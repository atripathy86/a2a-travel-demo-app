...
def _before_model_callback(
    ...
    """
    model_config = get_model_for_agent("budget")

    if model_config:
        llm_request.model = model_config.model
        print(f"💰 Budget using model: {model_config.name} ({model_config.model})")
...
Return ONLY valid JSON, no markdown code blocks, no other text.
    ...
    def _get_model(self):
        ...
        if not model_config:
            ...
            raise ValueError(
                "No model configuration found for budget agent. "
                "Check .env.budget.models.json file or set GOOGLE_API_KEY."
            )
...
def main():
    ...
    app = server.build()
    model_routes = create_starlette_model_routes("budget")
